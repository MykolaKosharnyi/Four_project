...
public class Dispatcher extends Thread{
	...
	private int	generateIdEmployee(Petition pet){
		...
		Employee resultEmployee = allEmployeeForCurrentJob.get(rand.nextInt(allEmployeeForCurrentJob.size()));
	...
	private Work generateWork(Petition pet, Brigade brigade){
		...
		work.setTime(pet.getTime());//подключить время по масштабу и за каждого работника
		work.setAdress(factory.createTenantDao().find(pet.getIdUser()).getAdress());//взять с юзера
